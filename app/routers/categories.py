...
from app.schemas import Category as CategorySchema, CategoryCreate, Category
...
async def delete_category(category_id: int, db: Session = Depends(get_db)):
    ...
    """
    stmt_name = select(CategoryModel.name).where(CategoryModel.id == category_id)
    name = db.scalars(stmt_name).first()
    ...
    db.commit()
    return {"status": "success", "message": f"'{name}' Category marked as inactive"}